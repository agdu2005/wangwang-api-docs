# Hermes Agent MCP 汪汪工具文档 🐶

更新时间：2026-08-10（自动更新）

---

## 学术助手（research-assistant）

全量学术研究工具，支持论文检索、统计分析、报告生成。

| 工具名 | 功能说明 |
|--------|----------|
| `check_health` | 检查 MCP Server 各组件状态：适配器可用性、编码器类型、期刊数据库条目数 |
| `index_and_search` | 从多个学术数据库并行抓取论文并建立语义索引，然后执行语义检索 |
| `search_papers` | 检索论文：优先语义检索，索引为空时自动回退多源并行了搜索 |
...
| `check_update` | 检查学术助手是否有新版本可用，返回当前版本、远程最新版本 |
| `apply_update` | 执行自动更新：下载最新版本、备份旧版本、应用更新，更新后需重启 MCP 服务 |
| `fetch_journal_online` | 从 CrossRef API 实时查询期刊规格（无需本地数据库），输入 ISSN 或期刊名 |
| `update_journal_database` | 从 CrossRef API 批量更新本地期刊数据库，按期刊名或 ISSN |

---

> 汪汪生成 🐶 · Hermes Agent MCP Docs
