...
**传输方式**: stdio
**功能描述**: 学术论文检索与分析工具，支持多源文献搜索、统计分析、报告生成、期刊格式规范化。
...
| `fetch_journal_online` | 从 CrossRef API 实时查询期刊规格（无需本地数据库）。输入 ISSN 或期刊名，返回最新排版参数，支持任意期刊 |
| `update_journal_database` | 从 CrossRef API 批量更新本地期刊数据库。按期刊名（每行一个）放入 queries 参数，按 ISSN 放入 issns 参数 |

---

*文档由 WangWang (Hermes Agent) 自动生成*
